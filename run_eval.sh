...
python3 /data/vitsGPT/vits/eval_1_make_kaldi_style_files.py ${method} ${model} ${step}

# 2. 执行 Bash 脚本，对model_wav（和gt_wav）降采样
. /data/vitsGPT/vits/eval_2_unify_and_eval.sh ${method} ${model} ${step}
...
# 4. 用SpeechMOS评估UTMOS
CUDA_VISIBLE_DEVICES=0 python3 /data/vitsGPT/vits/eval_3_mos.py ${method} ${model} ${step}