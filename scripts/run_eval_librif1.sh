...
python3 /data/vitsGPT/vits/eval_librif/eval_1_make_kaldi_style_files.py ${method} ${model} ${step}

# 2. 执行 Bash 脚本，对model_wav（和gt_wav）降采样
. /data/vitsGPT/vits/eval_librif/eval_2_unify_and_eval_init.sh ${method} ${model} ${step} # 第一次的话执行这个，后面就执行下面的了
...
# 4. 用SpeechMOS评估UTMOS
CUDA_VISIBLE_DEVICES=0 python3 /data/vitsGPT/vits/eval_librif/eval_3_mos.py ${method} ${model} ${step}